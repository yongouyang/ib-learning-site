...
| **Infra** | AWS S3 + CloudFront + Lambda + DynamoDB + Resend (transactional email; SES kept dormant), Terraform IaC, GitHub Actions CI/CD (OIDC) | **PROD = octavlearning.com** (www→apex 301), **DEV = dev.octavlearning.com**; two-tier deploy from main/develop. Email provider-swapped to Resend 2026-08-21 (SES production access denied twice; re-appeal open) |
