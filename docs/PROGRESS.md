...
Next: (1) **push to develop → deploy-dev applies the IAM fix, then verify**: request-otp to a SES-verified address should 200 + deliver; hotmail should 502 with `MessageRejected` (not AccessDenied) in CloudWatch. (2) SES appeal vs provider-swap decision still open (pre-launch blocker). (3) NEW finding (pre-existing, UX pass): markdown pipe tables in German/Chinese vocab notes render as raw `| --- |` text — StudyNoteBody has no table support; follow-up needed (render tables or restyle content). Then prior queue (topic ordering, Phase A A4).
