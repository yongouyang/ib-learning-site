...
variable "reserved_concurrent_executions" {
  description = "Reserved concurrency for the auth Lambda (brute-force/cost cap; set to 10 in terraform/envs/prod on 2026-09-18). Default null = unmanaged (shares the account pool — fine now that the ap-east-1 concurrent-executions quota (L-B99A9384) was raised 10 → 1000 in Sep 2026, which removed the reason this was forced to stay null). A non-null value both CAPS this function and GUARANTEES it that capacity, so set one only to isolate blast radius deliberately."
  type        = number
  default     = null
}
