...
export default function FlashcardsPage() {
  ...
  if (!topic || topic.flashcards.length === 0) return <div className="p-6 text-center text-gray-500">No flashcards available.</div>;

  if (isComplete) {
    return (
      <div className="max-w-md mx-auto px-4 py-8 text-center">
        <div className="text-5xl mb-4">🎉</div>
        <h1 className="text-xl font-bold text-gray-900 mb-2">Deck Complete!</h1>
        <p className="text-sm text-gray-500 mb-6">You reviewed all {topic.flashcards.length} flashcards.</p>
        <div className="flex gap-3 justify-center">
          ...
          <Link href={`/subjects/${subjectId}/${topicId}/quiz`}
            className="px-4 py-2 rounded-lg bg-orange-50 text-orange-700 font-medium text-sm hover:bg-orange-100 transition-colors">
            Take Quiz
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <span className="text-xs text-gray-500 shrink-0">{currentIndex + 1}/{topic.flashcards.length}</span>
      </div>
      ...
      <div onClick={() => setIsFlipped(!isFlipped)}
        ...
        {!isFlipped ? (
          <h2 className="text-xl font-bold text-gray-900">{card.term}</h2>
        ) : (
          <div>
      ...
      <div className="flex gap-3 mt-4">
        <button onClick={() => { setIsFlipped(false); setCurrentIndex((i) => Math.max(0, i - 1)); }}
          ...
          className="flex-1 py-3 rounded-xl bg-gray-100 text-gray-700 font-medium text-sm hover:bg-gray-200 transition-colors disabled:opacity-40">
          ← Previous
        </button>
        <button onClick={() => {
          setIsFlipped(false);
