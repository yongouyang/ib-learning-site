...
export default function QuizPage() {
  ...
  const [questions] = useState(() =>
    topic ? [...topic.questions].sort(() => Math.random() - 0.5) : []
  );
  ...
  const progress = questions.length > 0 ? ((currentIndex + (isComplete ? 1 : 0)) / questions.length) * 100 : 0;
  ...
  const handleNext = useCallback(() => {
    if (currentIndex < questions.length - 1) {
      ...
      if (!isComplete && topic) {
        recordAttempt(topicId, subjectId, topic.title, subjectId, correctCount, questions.length);
      }
      setIsComplete(true);
    }
  }, [currentIndex, isComplete, correctCount, questions.length, topic, topicId, subjectId, recordAttempt]);
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-8 text-center">
        ...
        <div className="flex gap-3 justify-center">
          <button onClick={() => { setCurrentIndex(0); setSelectedIndex(null); setAnswerState('unanswered'); setCorrectCount(0); setIsComplete(false); setShowExplanation(false); }}
  ...
  if (!currentQuestion) return <div className="p-6 text-center text-gray-500">No questions available.</div>;

  return (
    <div className="max-w-lg mx-auto px-4 py-6">
      {/* Progress bar */}
      <div className="flex items-center gap-3 mb-6">
        <Link href={`/subjects/${subjectId}`} className="text-sm text-blue-600 shrink-0">← Back</Link>
        <div className="flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
          <div className="h-full bg-blue-500 rounded-full transition-all duration-300" style={{ width: `${progress}%` }} />
        </div>
        <span className="text-xs text-gray-500 shrink-0">{currentIndex + 1}/{questions.length}</span>
      </div>

      {/* Question */}
      <div className="card p-5 mb-4">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">{currentQuestion.stem}</h2>
