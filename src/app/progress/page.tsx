    ...
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className="card p-4 text-center">
          ...
          <div className="text-xl font-black text-gray-900">{userProgress.totalStars}</div>
          <div className="text-xs text-gray-500">Total Stars</div>
        </div>
        <div className="card p-4 text-center">
          <div className="text-2xl mb-1">🔥</div>
          <div className="text-xl font-black text-gray-900">{userProgress.currentStreakDays}</div>
          <div className="text-xs text-gray-500">Day Streak</div>
        </div>
        <div className="card p-4 text-center">
          <div className="text-2xl mb-1">✅</div>
          <div className="text-xl font-black text-gray-900">{attemptedTopics}</div>
        ...
        return (
          <div key={subject.id} className="card p-4 mb-3">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-gray-500">{completed}/{subject.topics.length} topics</span>
            </div>
            {/* Progress bar */}
              ...
              {subject.topics.map((topic) => {
                ...
                return (
                  <Link key={topic.id} href={`/subjects/${subject.id}/${topic.id}/quiz`}
                    ...
                    <span className={`w-1.5 h-1.5 rounded-full ${score >= 0 ? '' : 'bg-gray-300'}`}
                      style={score >= 0 ? { backgroundColor: subject.accentColor } : undefined} />
                    <span className="flex-1 text-gray-700">{topic.title}</span>
                    {score >= 0 ? (
                      <span className="text-xs font-medium text-gray-500">{Math.round(score * 100)}%</span>
                    ) : (
