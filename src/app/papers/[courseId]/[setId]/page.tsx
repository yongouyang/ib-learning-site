...
export default async function PaperPage(props: { params: Promise<{ courseId: string; setId: string }> }) {
  ...
  // Metadata (title/description/params) comes from registry.ts; the questions and mark schemes
  // come from registry.papers.ts, which is server-only. Premium sets move to the gated content API
  // in Phase 1b — until then every set is served from the build, exactly as today.
