  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">IBLearn</h1>
          <p className="text-sm text-gray-500">{subjects.length} subjects · {subjects.reduce((s, sub) => s + sub.topics.length, 0)} topics</p>
        </div>
      ...
      {weakTopics.length > 0 && (
        <div className="card p-4 mb-6 bg-orange-50/50 border-orange-200">
          <h2 className="text-sm font-semibold text-orange-800 mb-2">Needs Practice</h2>
            ...
            {weakTopics.slice(0, 3).map((tp) => {
              ...
              return (
                <Link key={`${tp.subjectId}:${tp.topicId}`} href={`/subjects/${tp.subjectId}/${tp.topicId}/quiz`}
                  ...
                  <span className="text-orange-600 font-medium">{Math.round(getRecentAverageScore(tp.attempts) * 100)}%</span>
                </Link>
              );
            })}
          </div>
        ...
        {subjects.map((subject) => {
          ...
          return (
            <Link key={subject.id} href={`/subjects/${subject.id}`}
              ...
              <p className="text-xs text-gray-500 mb-2">{subject.topics.length} topics</p>
              <div className="flex gap-0.5">
                {[0, 1, 2].map((i) => (
                  <span key={i} className={`text-sm ${i < stars ? 'text-yellow-500' : 'text-gray-300'}`}>
                    {i < stars ? '★' : '☆'}
                  </span>
                ))}
              </div>
