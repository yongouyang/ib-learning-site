...
## 7. Custom domain (later, not in the first deploy)

- Register/transfer domain (Route 53 or external registrar).
- ACM cert in `us-east-1` (CloudFront requirement), alternate domain name on the distribution, DNS validation.
- Terraform: `domain` module, variables `domain_name`, `hosted_zone_id`. CORS + any absolute-URL references updated.
- Phase 7 §6 checks re-run against the real domain.
