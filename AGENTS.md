...
- Deploy: **two live environments since 2026-08-11** (custom-domain-cutover-plan.md): **PROD = https://octavlearning.com** (www → apex 301; bucket `iblearn-prod-site-305655353474`, distribution `EYC8IOH4L9SXT`; deploys from `main`) and **DEV = https://d2c1g77zfmjpm3.cloudfront.net** (bucket `iblearn-site-305655353474`, distribution `E1BMVEW6YOKNUY`; deploys from `develop`). Both proxy `/api/feedback` to the same feedback **Lambda** (Session 3), **live since 2026-08-09 with DeepSeek** (`deepseek-v4-flash`; Moonshot abandoned — `api.moonshot.cn` gets blackholed from AWS ap-east-1); Function URL CORS allows dev + apex + www origins. Unset `FEEDBACK_ENV` = unconfigured mode: GET `{configured:false}`, POST 501, "Mark with AI" hidden. Plan: `docs/aws-deployment-plan.md`; provider notes: `docs/ai-feedback.md`.
