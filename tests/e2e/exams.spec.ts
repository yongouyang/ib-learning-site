...
test.describe('Mock exams', () => {
  ...
  test('premium sessions see every mock unlocked and a full timed run records the result', async ({ page }) => {
    ...
    // Both practice sets per course are unlocked cross-links too (10 courses × 2).
    await expect(page.getByRole('link', { name: /— free-response/ })).toHaveCount(20);
  ...
  test('course cards cross-link to their free-response sets', async ({ page }) => {
    ...
    // in the DOM (10 inert previews) but are aria-hidden, so they are NOT
    // accessible links for anonymous visitors…
    await expect(links).toHaveCount(10);
    await expect(page.locator('a[href$="-set-2"]').filter({ hasText: /— free-response/ })).toHaveCount(10);
    ...
    await expect(page.getByRole('link', { name: /Premium · Full exam sets/ })).toHaveCount(10);
