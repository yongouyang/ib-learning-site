...
const server = http.createServer((req, res) => {
  ...
  const authRoute = AUTH_ROUTES[url.pathname] ?? PROGRESS_ROUTES[url.pathname] ?? ANALYTICS_ROUTES[url.pathname] ?? LEADERBOARD_ROUTES[url.pathname];
  if (authRoute) {
...
server.listen(port, () => {
  console.log(`[serve-static] Serving out/ + /api/feedback + /api/auth/* + /api/progress* + /api/analytics/* + /api/leaderboard* on http://localhost:${port}`);
});
